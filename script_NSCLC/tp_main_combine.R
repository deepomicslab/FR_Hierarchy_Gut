...
source('D:/fr_pkg_last/script_NSCLC/tp_helper.R')
library(pROC)
library(randomForest)
## 1. Discovery analysis set ----
log_msg('####### Discovery analysis set #########')
clin_disc <- load_clin(cohort = 'Disc', dir = 'D:/fr_pkg_last/data/immu')
### 1.5 Toposcoring ----
pred_disc <- read.table('D:/fr_pkg_last/result/immu/combine/pred_disc.tsv', header = TRUE, sep = '\t')
...
ycut_r <- youden[which.max(youden$THRESHOLD < 0.5), ]
temp = ycut_r
ycut_r = ycut_nr
ycut_nr = temp
...
ggsave(plt_roc, filename = 'D:/fr_pkg_last/result/immu/combine/fig_roc.pdf', width = 10, height = 10, units = "cm")
...
ggsave(plt_kde_disc, filename = 'D:/fr_pkg_last/result/immu/combine/fig_kde_disc.pdf', width = 10, height = 12, units = "cm")


### 1.6 Prediction in discovery cohort (full signature) ----
pred_disc <- read.table('D:/fr_pkg_last/result/immu/combine/pred_valid.tsv', header = TRUE, sep = '\t')
pred_disc = mutate(pred_disc, OS12=factor(OS12, levels = c('NR', 'R')))
...
# Prediction discovery: HR = 0.58 [0.43-0.80], p = 8.1e-04  Prediction discovery: HR = 0.69 [0.51-0.94], p = 1.8e-02
#fig_km_disc <- print_plot(plot_mykm(pred_disc, type = 'OS', by = 'PRED'))
#ggsave(fig_km_disc, filename = 'D:/fr_pkg_last/result/immu/combine/fig_km_disc.pdf', width = 10, height = 10, units = "cm")
