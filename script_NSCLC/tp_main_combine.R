...
### 1.5 Toposcoring ----
pred_disc <- read.table('../result/immu/combine/pred_disc.tsv', header = TRUE, sep = '\t')
...
ggsave(plt_roc, filename = '../result/immu/combine/fig_roc.pdf', width = 10, height = 10, units = "cm")
...
ggsave(plt_kde_disc, filename = '../result/immu/combine/fig_kde_disc.pdf', width = 12, height = 10, units = "cm")


### 1.6 Prediction in discovery cohort (full signature) ----
pred_disc <- read.table('../result/immu/combine/pred_valid.tsv', header = TRUE, sep = '\t')
pred_disc = mutate(pred_disc, OS12=factor(OS12, levels = c('NR', 'R')))
...
#ggsave(fig_km_disc, filename = '../result/immu/combine/fig_km_disc.pdf', width = 10, height = 10, units = "cm")
...
GRAPHICS$THEME
ggsave(p, filename = '../result/immu/combine/result.pdf', width = 10, height = 10, units = "cm")