...
# ggsave(fig_km_disc, filename = 'fig_km_disc.pdf', width = 10, height = 10, units = "cm")

pred_disc <- read.table('D:/fr_pkg_last/result/immu/combine/pred_binary_disc.tsv', header = TRUE, sep = '\t')
pred_disc = mutate(pred_disc, OS12=factor(OS12, levels = c('NR', 'R')))
rf_model <- randomForest(OS12 ~ TOPOB01, data = pred_disc)
