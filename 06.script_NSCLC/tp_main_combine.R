...
### 1.5 Toposcoring ----
pred_disc <- read.table('../result/NSCLC/combine/pred_disc.tsv', header = TRUE, sep = '\t')
...
ggsave(plt_roc, filename = '../result/NSCLC/combine/fig_roc.pdf', width = 10, height = 10, units = "cm")
...
ggsave(plt_kde_disc, filename = '../result/NSCLC/combine/fig_kde_disc.pdf', width = 12, height = 10, units = "cm")


### 1.6 Prediction in discovery cohort (full signature) ----
pred_disc <- read.table('../result/NSCLC/combine/pred_valid.tsv', header = TRUE, sep = '\t')
pred_disc = mutate(pred_disc, OS12=factor(OS12, levels = c('NR', 'R')))
...
#ggsave(fig_km_disc, filename = '../result/NSCLC/combine/fig_km_disc.pdf', width = 10, height = 10, units = "cm")
...
geom_step() + geom_abline(slope = 1, intercept = 0, linetype = 'dashed', color = 'blue') +
xlim(0, 1) + ylim(0, 1) +
xlab('1 - Specificity') + ylab('Sensitivity') +
GRAPHICS$THEME
ggsave(p, filename = '../result/NSCLC/combine/result.pdf', width = 10, height = 10, units = "cm")