...
if __name__ == '__main__':
    ...
    # read file
    eigengene_df = pd.read_csv(os.path.join(odir, pheno, '{}.together.eigengene.csv'.format(prefix)),index_col=0,header=0,sep="\t")
...
# select selected_col_list columns from metadata
selected_metadata_df = metadata_df[['sample_id'] + selected_col_list]

# compute module and selected_col_list pearson and p_value for each column
results = []
for module in eigengene_df['module'].unique():
    module_df = eigengene_df[eigengene_df['module'] == module]
    
    # merge eigengeneand metadata
    merged_df = module_df.merge(selected_metadata_df, left_on='sample', right_on='sample_id', how='inner')
    
    for col in selected_col_list:
        valid_rows = merged_df[[col, 'eigengene']].dropna()

        corr, p_value = pearsonr(valid_rows[col], valid_rows['eigengene'])
