...
if __name__ == '__main__':
    ...
    # read file
    eigensp_df = pd.read_csv(os.path.join(odir, '{}.eigenspecies.csv'.format(prefix)),index_col=0,header=0,sep="\t")
    metadata_df = pd.read_csv(os.path.join(idir, pheno, prefix,'metadata.tsv'), sep='\t', header=0)
    metadata_stat_path = os.path.join(odir, '{}.metadata.stat.tsv'.format(prefix))
...
_, fdr_p_values = fdrcorrection(result_df['p-value'].values)
result_df['FDR p-value'] = fdr_p_values
result_df.to_csv(os.path.join(odir, '{}.FRC_correlation.tsv'.format(prefix)), index=False,sep="\t")
